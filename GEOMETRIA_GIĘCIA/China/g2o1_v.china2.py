...
import sys
import string
import math  # Dodano import modułu math do obliczeń trygonometrycznych
...
def calculate_arc_length(radius, angle_deg):
    """
    Oblicza długość łuku dla danego promienia i kąta (w stopniach).
    """
...
def process_file(filename, output_dir):
    ...
    wynik_lines.append("-------------------------------------------")
    out_values = {}
    a_out = int(round(mainplane_value + total_offset * math.tan(math.radians(180 - bends_info[0]['angle_norm'])) / 2))
    out_values["a"] = a_out

    for sc_name in sorted_sc_names:
        label = sc_labels[sc_name]
        val_zewn = sc_components[sc_name]
        val_out = int(round(val_zewn + total_offset * math.tan(math.radians(180 - bends_info[0]['angle_norm'])) / 2))
    ...
    arc_length = calculate_arc_length(neutral_radius, bends_info[0]['angle_norm'])
    a_in = int(round(mainplane_value + arc_length / 2))
    in_values["a"] = a_in

    for sc_name in sorted_sc_names:
        label = sc_labels[sc_name]
        val_in = int(round(sc_components[sc_name] + arc_length / 2))
